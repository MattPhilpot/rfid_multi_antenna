...
ipj_error ipj_util_cleanup(ipj_iri_device* iri_device, ipj_error* inventory_error)
{
    ipj_error error;

    /* Reset the IRI device */
    if (inventory_error != NULL && inventory_error > 0)
    {
        error = ipj_reset(iri_device, E_IPJ_RESET_TYPE_FACTORY_RESTORE);
        IPJ_UTIL_RETURN_ON_ERROR(error, "ipj_reset E_IPJ_RESET_TYPE_FACTORY_RESTORE");
    }
    else
    {
        ...
        uint32_t* tag_operation_report)
{
    ...
    while (index < report_count_32)
    {
        ...
        switch (REPORT_FIELD_ID(field_header))
        {
            ...
            case E_IPJ_TAG_OPERATION_REPORT_RSSI:
            {
                ...
                rssi = (int)tag_operation_report[index];
                break;
            }
            ...
            {
                //printf("ANTENNA: %d\n", tag_operation_report[index]);
                antenna = tag_operation_report[index];
                break;
            }
    ...
    toAdd.rssi[antenna - 1] = rssi;
    insertTagIntoLatestHistory(toAdd, antenna);
